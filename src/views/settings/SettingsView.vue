...
import { applyTheme, getStoredTheme, saveTheme } from '@/composables/useTheme';
...
// 보기 설정
const displaySetting = ref(getStoredTheme());
...
// 테마 변경 감지 — 고른 값을 그대로 저장하고, 화면에는 환산된 값을 적용한다.
// ('system' 은 저장은 'system' 으로, 적용은 OS 설정에 따라 light/dark 로)
watch(displaySetting, (newValue) => {
  saveTheme(newValue);
});

onMounted(() => {
  displaySetting.value = getStoredTheme();
  applyTheme(displaySetting.value);
});
