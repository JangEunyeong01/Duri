...
import { getCardRecommendations as fetchCardRecommendations } from '@/api/walletApi';
...
const handlePasswordSuccess = async () => {
  ...
  try {
    ...
    const errorMsg = getErrorMessage(error, 'QR 생성에 실패했습니다');
...
<template>
  <div class="payment-page">
    ...
    <!-- 이용 장소 선택 -->
    <div v-if="isLogin && hasCards" class="merchant-section" style="margin-top: var(--space-md); margin-bottom: var(--space-md);">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-sm);">
        <h3 style="margin: 0; font-size: var(--typo-heading-3-size); font-weight: 700; color: var(--color-text-primary);">이용 장소</h3>
        <button
          @click="router.push('/settings/personalization')"
          style="background: none; border: none; color: var(--color-text-secondary); font-size: var(--typo-caption-large-size); cursor: pointer; padding: var(--space-xxs) var(--space-xs); text-decoration: underline; transition: color var(--transition-normal);"
    ...
    <div v-if="hasCards" id="payment-info-section" class="payment-amount-section" style="margin-bottom: var(--space-md);">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-sm);">
        <h3 style="margin: 0; font-size: var(--typo-heading-3-size); font-weight: 700; color: var(--color-text-primary);">결제 금액</h3>
        <button
          @click="showPaymentAmountInput = !showPaymentAmountInput"
          style="background: none; border: none; font-size: var(--typo-heading-2-size); cursor: pointer; padding: var(--space-xxs) var(--space-xs); color: var(--color-text-primary); display: flex; align-items: center; justify-content: center; transition: transform var(--transition-slow);"
    ...
    <div id="recommendation-section" v-if="hasCards" style="margin-bottom: var(--space-md); padding: 0 var(--space-md);">
      <div style="background: linear-gradient(135deg, rgba(var(--color-primary-rgb), 0.1) 0%, rgba(var(--color-primary-rgb), 0.05) 100%); border-radius: var(--radius-lg); padding: var(--space-md); margin-bottom: var(--space-sm);">
        <h4 style="margin: 0 0 8px 0; font-size: var(--typo-label-large-size); font-weight: 700; color: var(--color-text-primary);">🎯 추천 카드 찾기</h4>
        <p style="margin: 0 0 12px 0; font-size: var(--typo-caption-large-size); color: var(--color-text-secondary);">카테고리와 금액에 맞는 최적의 카드를 추천받으세요</p>
        <button
          @click="getCardRecommendations"
          :disabled="isRecommending"
          style="width: 100%; padding: var(--space-sm) var(--space-md); background: var(--color-primary); color: var(--color-btn-primary-text); border: none; border-radius: var(--radius-md); font-weight: 700; font-size: var(--typo-label-large-size); cursor: pointer; transition: all var(--transition-normal);"
    ...
    <div v-if="!isLogin" style="flex: 1; display: flex; align-items: center; justify-content: center;">
      <div style="text-align: center; display: flex; flex-direction: column; gap: var(--space-md);">
        <h2 style="margin: 0; font-size: var(--typo-heading-2-size); font-weight: 700; color: var(--color-text-primary);">로그인이 필요합니다</h2>
        <p style="margin: 0; font-size: var(--typo-body-medium-size); color: var(--color-text-secondary);">결제하려면 로그인해주세요.</p>
        <button @click="goLogin" style="padding: var(--space-sm) var(--space-lg); background: var(--color-primary); color: var(--color-btn-primary-text); border: none; border-radius: var(--radius-full); font-weight: 600; cursor: pointer;">로그인</button>
      </div>
    </div>
    ...
    <main v-else style="flex: 1; display: flex; flex-direction: column;">
      ...
      <div v-if="!hasCards" style="flex: 1; display: flex; align-items: center; justify-content: center;">
        <div style="text-align: center; display: flex; flex-direction: column; gap: var(--space-md); width: 100%; padding: 0 var(--space-md); box-sizing: border-box;">
          <h2 style="margin: 0; font-size: var(--typo-heading-2-size); font-weight: 700; color: var(--color-text-primary);">등록된 카드가 없어요</h2>
          <p style="margin: 0; font-size: var(--typo-body-medium-size); color: var(--color-text-secondary);">카드를 등록하면 혜택과 소비 관리를 시작할 수 있습니다.</p>
          <button @click="$router.push('/cards/register')" style="width: 100%; padding: var(--space-sm) var(--space-lg); background: var(--color-primary); color: var(--color-btn-primary-text); border: none; border-radius: var(--radius-full); font-weight: 600; cursor: pointer; box-sizing: border-box;">카드 등록</button>
        </div>
      </div>
      ...
      <div class="payment-carousel-area">
        ...
        <div v-show="hasCards && recommendedCardIds.length > 0 && currentCard && recommendedInfo?.[currentCard?.id]"
          ...
          <p style="margin: 0 0 12px 0; font-size: var(--typo-label-large-size); font-weight: 600; color: var(--color-text-primary);">추천 결과</p>
          <h3 class="benefits-title">
            {{ recommendedInfo?.[currentCard?.id]?.rank }}위 · {{ currentCard?.name }}
            <span style="font-size: var(--typo-caption-large-size); font-weight: 500; color: var(--color-text-secondary); margin-left: var(--space-xs);">
        ...
        <div v-show="hasCards && recommendedCardIds.length === 0 && currentCard"
             class="benefits-section">
          <p style="margin: 0 0 12px 0; font-size: var(--typo-label-large-size); font-weight: 600; color: var(--color-text-primary);">카드 주요 혜택</p>
          <h3 class="benefits-title">
            {{ currentCard?.name }}
            <span style="font-size: var(--typo-caption-large-size); font-weight: 500; color: var(--color-text-secondary); margin-left: var(--space-xs);">
              [본인] {{ currentCard?.cardNumber?.slice(-4, -1) }}*
        ...
        <!-- 스와이프 가이드 톨팁 -->
        <div v-if="showSwipeGuide" style="position: absolute; top: 20px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.8); color: white; padding: var(--space-xs) var(--space-md); border-radius: var(--radius-lg); font-size: var(--typo-caption-large-size); z-index: var(--z-dropdown); white-space: nowrap; animation: fadeInOut 3s;">
...
.section-title {
  ...
  font-size: var(--typo-heading-3-size);
  font-weight: 700;
...
.payment-btn {
  ...
  transform: scale(1);
   margin-top: auto;   /* 자동으로 아래쪽으로 밀림 */
    margin-bottom: var(--space-lg); /* 네비게이션 위에 여백 */
}
